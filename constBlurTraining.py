...
from kornia.augmentation import (RandomHorizontalFlip,
                                 ...
                                 RandomMotionBlur,
                                 RandomGaussianBlur)
from kornia.filters import GaussianBlur2d, BoxBlur, MedianBlur


def const_blur_train(sigma1, kernelsize):
    ...
    # blur_img = BoxBlur([kernelsize, kernelsize])
    # blur_img = MedianBlur([kernelsize, kernelsize])
    # blur_img = RandomMotionBlur(kernel_size=int(kernelsize), angle=360, direction=0, p=1)
    blur_img = RandomGaussianBlur((9, 9), sigma=(3, 3), p=1)
    params = {
        "lr": lr,
    ...
    for epoch in tqdm.tqdm(range(epochs), total=epochs):  # loop over the dataset multiple times
        ...
        # Change Model name by adding current epoch into the name
        MODEL_PATH_EXTENDED = MODEL_PATH + f"_curr_{epoch + 1}.pth"
        if min_valid_loss > valid_loss:
            print(f'Validation Loss Decreased({min_valid_loss:.6f}--->{valid_loss:.6f}) \t Saving The Model')
            min_valid_loss = valid_loss
            # Saving State Dict
            # torch.save(net.state_dict(), MODEL_PATH)
        # Save the network after every 10 epochs
        if epoch_counter == 10:
...
MODEL_PATH = f'random_gauss_blur_lr{lr}_epochs{epochs}_prob1_range0_3'
description = f"random_gauss_blur_lr{lr}_epochs{epochs}_prob1_range0_3"
...
""" If a pre-trained network is to be used, load state here """
# net.load_state_dict(torch.load("const_motion_blur_training_continued_ks9_to_5_200_epochs_curr_60.pth"))

""" Initialize training; choose blur filter in function before """
const_blur_train(sigma1=sigma1, kernelsize=kernelsize)
